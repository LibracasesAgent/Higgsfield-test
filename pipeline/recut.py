...
def source(s):
    return direct_url(s) if DRIVE_ID.match(s) and not os.path.exists(s) else s
...
def render_segment(seg, i, tmp, hook, audio_mode):
    ...
    cmd = ["ffmpeg", "-hide_banner", "-loglevel", "error", "-ss", str(seg.get("start", 0)),
           "-t", f"{dur * speed:.3f}", "-i", source(seg["src"]), "-loop", "1", "-t", f"{dur:.3f}", "-i", ov,
